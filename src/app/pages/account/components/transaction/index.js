...
import { Avatar, Button, Col, Layout, Row, Table, Typography } from 'antd'
import { MSG } from 'app/consts'
import coin from 'app/consts/coin'
import { onSetReceiveAssetState, onSetSendAssetState } from 'app/pages/account/redux/slice'
...
import { formatAmount, formatDateTime, pDecimalBalance } from 'app/utils/format'
    ...
    async (accountSelected) => {
      ...
      if (history.status === MSG.SUCCESS) {
        ...
        const fmt = data.map((item) => {
          const { txId, txType, lockTime, status, nativeTokenInfo, privacyTokenInfo, accountPublicKeySerialized, historyType } = item
          const payment = {
            fee: null,
            amount: null,
            address: null,
